...
// ── QR KOD İLƏ DAVAMIYYƏT ────────────────────────────────────────

API.validateByQR = async (secret, dept, ts) => {
  if (!secret || !dept || !ts) return { valid: false, reason: 'Məlumatlar natamamdır.' };
  if (Math.abs(Date.now() - Number(ts)) > 60000)
    return { valid: false, reason: 'QR kodun vaxtı keçib. Yenidən skan edin.' };
  const validDepts = ['Elmlər', 'Sahil', 'Gənclik', 'Ağ Şəhər'];
  if (!validDepts.includes(dept)) return { valid: false, reason: 'Etibarsız filial QR kodu.' };

  const { data: emp } = await sb.from('employees').select('*').eq('secret', secret).single();
  if (!emp) return { valid: false, reason: 'İşçi tapılmadı.' };

  const ts2     = new Date();
  const todayStr = U.getLogicalDateStr(ts2);
  const todayYMD = U.getLogicalYMD(ts2);

  const todayShift = await U.getEmployeeShift(emp.id, todayYMD);
  if (todayShift === 'istirahetsm') return { valid: false, reason: 'Bu gün sizin istirahət gününüzdür!' };
  if (await U.hasApprovedLeave(emp.id, todayYMD)) return { valid: false, reason: 'Bu gün üçün təsdiq edilmiş izniniz var.' };

  const { data: allLogs } = await sb.from('attendance').select('*').eq('emp_id', String(emp.id));
  const todayLogs = (allLogs || []).filter(r => U.getLogicalDateStr(new Date(r.timestamp)) === todayStr);
  const shiftInfo = todayShift ? U.getShiftInfo(emp.dept, todayShift) : null;

  if (todayLogs.length === 0) {
    const nowMins = ts2.getHours() * 60 + ts2.getMinutes() +
      (ts2.getHours() < 3 && shiftInfo && shiftInfo.startH >= 12 ? 24 * 60 : 0);
    let late = shiftInfo
      ? nowMins > (shiftInfo.lateH * 60 + shiftInfo.lateM)
      : U.isLate(emp.dept, ts2);
    if (late) {
      const perm = await U.getApprovedLatePerm(emp.id, todayYMD);
      if (perm) {
        const [ph, pm] = perm.requestedTime.split(':').map(Number);
        if ((ts2.getHours() * 60 + ts2.getMinutes()) <= ph * 60 + pm + 5) late = false;
      }
    }
    const lateStr = late
      ? `\n⚠️ <b>GECİKMƏ!</b>${shiftInfo ? ` (limit: ${String(shiftInfo.lateH).padStart(2,'0')}:${String(shiftInfo.lateM).padStart(2,'0')})` : ''}`
      : '\n✅ Vaxtında';
    await sb.from('attendance').insert({
      emp_id: emp.id, emp_name: emp.name, dept: emp.dept,
      timestamp: ts2.toISOString(), type: 'GƏLİŞ', overtime: '', shift_type: todayShift || '',
    });
    await U.sendTelegramMsg(`<b>Smendə</b>\n <b>${emp.name}</b>\n${lateStr}\n ${U.fmtTime(ts2)}`, emp.dept);
    return { valid: true, empName: emp.name, dept: emp.dept, type: 'GƏLİŞ', overtime: '' };

  } else if (todayLogs.length === 1) {
    const reqH = shiftInfo ? shiftInfo.durH
      : ((emp.dept === 'Ağ Şəhər' || emp.dept === 'Gənclik') ? 9 : 8);
    const diffMs = ts2.getTime() - new Date(todayLogs[0].timestamp).getTime() - reqH * 3600000;
    const absMs  = Math.abs(diffMs);
    const dh = Math.floor(absMs / 3600000), dm = Math.floor((absMs % 3600000) / 60000);
    const overtimeStr = (dh === 0 && dm === 0) ? 'Tam vaxtında'
      : `${diffMs >= 0 ? '+' : '-'}${dh} saat ${dm} dəq`;
    await sb.from('attendance').insert({
      emp_id: emp.id, emp_name: emp.name, dept: emp.dept,
      timestamp: ts2.toISOString(), type: 'CIXIS', overtime: overtimeStr, shift_type: todayShift || '',
    });
    const otE = diffMs > 0 ? '🟢' : diffMs < 0 ? '🔴' : '⚪';
    await U.sendTelegramMsg(`<b>Smen bitdi</b>\n <b>${emp.name}</b>\n ${U.fmtTime(ts2)}\n${otE} Fərq: <b>${overtimeStr}</b>`, emp.dept);
    return { valid: true, empName: emp.name, dept: emp.dept, type: 'CIXIS', overtime: overtimeStr };
  }
  return { valid: false, reason: 'Bu gün üçün artıq qeyd var.' };
};
